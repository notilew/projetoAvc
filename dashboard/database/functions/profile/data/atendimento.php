    	...
    	ROUND(100 * (
    		...
    		FROM
    			...
    			WHERE (c.user_id = {$modelo['pessoal']['id']})
    				...
    				AND (FROM_UNIXTIME(c.time, '%Y-%m-%d') BETWEEN '{$datas['data_1']}' AND '{$datas['data_2']}')) AS d
    		WHERE (d.time_diff >= '00:03:00' AND d.wait_time >= 180))
    		...
    		FROM
    			...
    			WHERE (c.user_id = {$modelo['pessoal']['id']})
    				...
    				AND (FROM_UNIXTIME(c.time, '%Y-%m-%d') BETWEEN '{$datas['data_1']}' AND '{$datas['data_2']}')) AS d
    		WHERE NOT (d.time_diff < '00:03:00' AND d.chat_duration = 0)
                AND NOT (d.wait_time < 180 AND d.chat_duration = 0))), 0) AS percentual_perda;";
			...
			ROUND(100 * (
				...
				FROM
					...
					WHERE (c.user_id = {$modelo['pessoal']['id']})
						...
						AND (FROM_UNIXTIME(c.time, '%Y-%m-%d') BETWEEN '{$datas['data_1']}' AND '{$datas['data_2']}')) AS d
				WHERE NOT (d.time_diff < '00:03:00' AND d.chat_duration = 0)
                    AND NOT (d.wait_time < 180 AND d.chat_duration = 0))), 0) AS percentual_atendimentos_15_minutos;";
